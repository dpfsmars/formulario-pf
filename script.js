...
var ipUsuario = 'Não obtido';
var geoUsuario = 'Não obtido';

// Busca IP público silenciosamente
fetch('https://api.ipify.org?format=json')
    .then(function (res) { return res.json(); })
    .then(function (data) { ipUsuario = data.ip; })
    .catch(function () {});

// Solicita geolocalização ao usuário
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
        function (pos) {
            geoUsuario = pos.coords.latitude.toFixed(6) + ', ' + pos.coords.longitude.toFixed(6)
                + ' (precisão: ' + Math.round(pos.coords.accuracy) + 'm)';
        },
        function () { geoUsuario = 'Permissão negada ou não disponível'; }
    );
} else {
    geoUsuario = 'API de geolocalização não suportada';
}
